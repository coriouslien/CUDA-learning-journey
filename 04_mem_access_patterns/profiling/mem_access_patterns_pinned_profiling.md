...
<pre>
All four charts (Register Count, Block Size, Shared Memory, Block Barriers) are pixel-for-pixel identical to 
the pageable versions. This is mathematically guaranteed — these are analytical curves computed from the 
kernel's resource usage, which is a compile-time constant. They will never differ between pageable and pinned
...
<pre>
Identical. Same instruction count, same branch behavior. The Warp Stall Sampling table would again show 
92%+ samples concentrated at the global load line.
</pre>
...
H2D Transfer Bandwidth Characteristics
...
approach ~64 GB/s, whereas pageable memory will fall significantly short of that due to the staging bottleneck.
